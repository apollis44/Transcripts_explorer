...
def predict_from_fasta(fasta_path, output_dir, stats):
    ...
    # --- THE RAM DISK REDIRECT ---
    # Create a unique temporary folder in Linux shared memory RAM
    import uuid
    import shutil
    ram_embeddings_dir = f"/dev/shm/deeptmhmm_embs_{uuid.uuid4().hex}"

    try:
        # Generate embeddings directly into RAM-backed file storage
        generate_esm_embeddings(
            sequences=input_sequences,
            esm_embeddings_dir=ram_embeddings_dir,
            esm_model=esm_model,
        ...
        for model in models:
            model.esm_embeddings_dir = ram_embeddings_dir
        ...
        if os.path.exists(ram_embeddings_dir):
            shutil.rmtree(ram_embeddings_dir)